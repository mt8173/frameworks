...
public class PhoneStatusBar extends BaseStatusBar implements DemoMode,
    ...
    private final class SettingObserver extends ContentObserver {
        ...
        public void onChange(boolean selfChange, Uri uri) {
			...
			int mShowPercent = Settings.Global.getInt(mContext.getContentResolver(),
		   ...
		   mNetState = Settings.Global.getInt(mContext.getContentResolver(),
                SHOW_NETSPEED, 0);	
			if(mNetSpeedText !=null){
				if(mNetState==1){
					...
					mNetSpeedText.setVisibility(View.GONE);
				}		
			}
    ...
    protected PhoneStatusBarView makeStatusBarView() {
		...
		mBatteryLevel = (TextView) mStatusBarView.findViewById(R.id.percentage);//add code 
		...
		context.getContentResolver().registerContentObserver(
                Settings.Global.getUriFor(SHOW_PERCENT_SETTING), false, mSettingObserver);
		...
		//net speed watch
		try {
			mNetSpeedText = (TextView) mStatusBarWindow.findViewById(R.id.main_top_net_txt);
			mNetHandler = new Handler() {

				@Override
				public void handleMessage(Message msg) {
					if (msg.what == NETSPEEDSINGNAL) {
						mNetSpeedText.setText(msg.obj+"");
					}
					super.handleMessage(msg);
				}

			};
			mSpeed = NetSpeed.getInstance(mContext, mNetHandler);
			mSpeed.startCalculateNetSpeed();
		} catch (Exception e) {
			e.printStackTrace();
		}
		context.getContentResolver().registerContentObserver(
                Settings.Global.getUriFor(SHOW_NETSPEED), false, mSettingObserver);
		mRegisterNetworkReceiver(context);
	...
	private void mRegisterNetworkReceiver(Context context){
