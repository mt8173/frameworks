        ...
        public boolean onActionItemClicked(ActionMode mode, MenuItem item) {
            ...
            switch (item.getItemId()) {
				...
				case R.id.detail:
					...
					R.layout.details_dialog, null, false);
					TextView size = (TextView)detailsView.findViewById(R.id.size);
					TextView time = (TextView)detailsView.findViewById(R.id.last_time);
					TextView name_count = (TextView)detailsView.findViewById(R.id.count);
					String details = getResources().getString(R.string.detail);
					String file_size = getResources().getString(R.string.file_size);
					if(selection.size()==1){
						String name = getResources().getString(R.string.file_name);
						String lastTime = getResources().getString(R.string.file_last_time);
						DocumentInfo info = mModel.getDocuments(selection).get(0);
						name_count.setText(name+info.displayName);
						size.setText(file_size+info.size + "b");
						SimpleDateFormat sdf = new SimpleDateFormat("yyyy/MM/dd/ hh:mm:ss");						
						time.setText(lastTime+sdf.format(info.lastModified));
					}else{
						String count = getResources().getString(R.string.count);
						name_count.setText(count + selection.size());
						long overall_size = 0;
						for(int i = 0;i<selection.size();i++){
							overall_size += mModel.getDocuments(selection).get(i).size;
						}
						size.setText(file_size +overall_size + "b");
						time.setVisibility(View.GONE);
					}					
					Dialog mDialog = builder.setTitle(details).setView(detailsView).create();
					mDialog.show();
