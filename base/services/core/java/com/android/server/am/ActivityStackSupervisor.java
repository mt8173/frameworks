    ...
    void moveTasksToFullscreenStackLocked(int fromStackId, boolean onTop) {
        ...
        try {
            ...
            if ("1".equals(SystemProperties.get("ro.globalpq.support"))) {
                if (fromStackId == DOCKED_STACK_ID) {
                    String mForegroundAP = null;
                    if (onTop && size >0) {
                        mForegroundAP = tasks.get(0).affinity;
                    } else if (!onTop) {
                        ActivityStack mFullScreenStack = getStack(FULLSCREEN_WORKSPACE_STACK_ID);
                        if (mFullScreenStack != null) {
                            ArrayList<TaskRecord> mFullScreenTasks = mFullScreenStack.getAllTasks();
                            int mFullScreenSize = mFullScreenTasks.size();
                            if (mFullScreenSize >0) {
                                mForegroundAP = mFullScreenTasks.get(0).affinity;
                            }
                        }
                    }
                    if (mForegroundAP != null) {
