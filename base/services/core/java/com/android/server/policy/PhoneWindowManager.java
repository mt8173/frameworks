...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    public void init(Context context, IWindowManager windowManager,
        ...
        //by xin
        KeyMappingManager.registerListener(context);
    ...
    public long interceptKeyBeforeDispatching(WindowState win, KeyEvent event, int policyFlags) {
        ...
        if (keyCode == KeyEvent.KEYCODE_HOME) {
			...
			if (down && repeatCount == 0) {
                appSwitchFirstDown(keyguardOn);
            } else if (!down) {
            	appSwitchUp(keyguardOn);
            }
            return -1;
        ...
        //by xin 
        else if(keyCode ==KeyMappingManager.key){
            if(down){
                if(repeatCount==0){
                    isLongPressKeyMapping=false;
                }else if(repeatCount==1){
                    isLongPressKeyMapping=true;
                }
            }else if(event.getAction() == KeyEvent.ACTION_UP){
                Log.i("xin_log","count="+repeatCount+",long="+isLongPressKeyMapping);
                if(isLongPressKeyMapping){
                    mHandler.postDelayed(keyMappingClearRunnable,50);
                }else{
                    mHandler.postDelayed(keyMappingRunnable,50);
                }
                isLongPressKeyMapping=false;
            }
            return -1;
	...
	private boolean menuSending=false;
    public void appSwitchFirstDown(boolean keyguardOn){
    	if(!keyguardOn){
    		preloadRecentApps();
    	}
    	menuSending=true;
    	mHandler.postDelayed(mSendMenu, ViewConfiguration.getGlobalActionKeyTimeout());
    }
    public void appSwitchUp(boolean keyguardOn){
    	if(menuSending){
    		mHandler.removeCallbacks(mSendMenu);
    		if(!keyguardOn){
        		toggleRecentApps();
        	}
    		menuSending=false;
    ...
    Runnable mSendMenu=new Runnable() {
    ...
    public int interceptKeyBeforeQueueing(KeyEvent event, int policyFlags) {
        ...
        final boolean canceled = event.isCanceled();
        final int keyCode = event.getKeyCode();
		final boolean isFingerprintKeyEvent = (event.getKeyboardDevice() == 5);
        ...
        if (DEBUG_INPUT) {
            Log.d(TAG, "interceptKeyTq keycode=" + keyCode + " scanCode= " + event.getScanCode() + " getKeyboardDevice= " + event.getKeyboardDevice()
        ...
        if (useHapticFeedback || isFingerprintKeyEvent) {
            performHapticFeedbackLw(null, HapticFeedbackConstants.CLOCK_TICK, false);
        }
    ...
    public int rotationForOrientationLw(int orientation, int lastRotation) {
        ...
        if (mForceDefaultOrientation) {
			if(SystemProperties.getBoolean("ro.wisky.xds", false)){
				return Surface.ROTATION_90;
			}else{
				return Surface.ROTATION_0;
			}
        ...
        synchronized (mLock) {
            ...
            if (mLidState == LID_OPEN && mLidOpenRotation >= 0) {
                ...
                if (sensorRotation != Surface.ROTATION_270
            ...
            switch (orientation) {
                ...
                default:
                    ...
                    //modify by linqihan@wisky
					if(SystemProperties.getInt("ro.sf.hwrotation", 0) == 270 ||SystemProperties.getBoolean("ro.wisky.xds", false)){
						return Surface.ROTATION_90;
					}else{
						return Surface.ROTATION_0;
					}
...
//==================================by xin
    private boolean isLongPressKeyMapping=false;
    Runnable keyMappingRunnable = new Runnable() {
        public void run() {

            performHapticFeedbackLw(null, HapticFeedbackConstants.LONG_PRESS, false);
            sendCloseSystemWindows(SYSTEM_DIALOG_REASON_RECENT_APPS);
            KeyMappingManager.launchKeyMappingUIByDirect(mContext, true);
        }
    };
    Runnable keyMappingClearRunnable = new Runnable() {
        public void run() {

                performHapticFeedbackLw(null, HapticFeedbackConstants.LONG_PRESS, false);
                KeyMappingManager.cleanCurrentMapping(mContext);
