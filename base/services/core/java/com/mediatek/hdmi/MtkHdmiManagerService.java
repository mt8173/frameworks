...
public final class MtkHdmiManagerService extends IMtkHdmiManager.Stub {
    ...
    public static final class Lifecycle extends SystemService {
        private MtkHdmiManagerService mMtkHdmiManagerService;
        private final Context mContext;
        ...
        @Override
        public void onStart() {
            mMtkHdmiManagerService = new MtkHdmiManagerService(mContext);
            publishBinderService(Context.HDMI_SERVICE,mMtkHdmiManagerService);
        }
    }
    ...
    private int getSuitableResolution(int videoResolution) {
        ...
        if (needUpdate(videoResolution)) {
            ...
            if (mEdid != null) {
                ...
                int edidTemp4k2k = mEdid[2];
                int edidTempband = 0;
                if (FeatureOption.MTK_SLIMPORT_COLORADO3){
                    edidTempband = mEdid[3];
                }
                int index = 0;
                ...
                for (int res : prefered) {
                    ...
                    log(TAG, "getSuitableResolution: " + act+" edidTempband: "+edidTempband);
                    if (act < HdmiDef.RESOLUTION_3840X2160P23_976HZ) {
                        if (0 != (edidTemp & HdmiDef.sResolutionMask[act])
                            && resolutionList.contains(act)) {
                            if (FeatureOption.MTK_SLIMPORT_COLORADO3){
                                if (edidTempband >= 5400){
                                    videoResolution = HdmiDef.AUTO + HdmiDef.RESOLUTION_1920X1080P_60HZ;
                                    break;
                                } else if (edidTempband >= 2700) {
                                    videoResolution = HdmiDef.AUTO + HdmiDef.RESOLUTION_1280X720P_60HZ;
                                    break;
                                } else {
                                    videoResolution = HdmiDef.AUTO + HdmiDef.RESOLUTION_720X480P_60HZ;
                                    break;
                                }
                            } else {
                                videoResolution = res;
                        ...
                        if (0 != (edidTemp4k2k & HdmiDef.sResolutionMask4k2k[act
                            - HdmiDef.RESOLUTION_3840X2160P23_976HZ])
                            && resolutionList.contains(act)) {
                                if (FeatureOption.MTK_SLIMPORT_COLORADO3){
                                    if (edidTempband >= 5400){
                                        videoResolution = res;
        ...
        for (int res : resolutions) {
            try {
                if (res >= HdmiDef.RESOLUTION_3840X2160P23_976HZ) {
                    int mask4k2k = HdmiDef.sResolutionMask4k2k[res
                    - HdmiDef.RESOLUTION_3840X2160P23_976HZ];
                    if ((edidTemp4k2k & mask4k2k) != 0) {
    ...
    private static class FeatureOption {
        ...
        public static final boolean MTK_SLIMPORT_COLORADO3 =
                getValue("ro.slimport.colorado3");
