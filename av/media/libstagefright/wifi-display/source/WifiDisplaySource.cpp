...
WifiDisplaySource::WifiDisplaySource(
    ...
    ALOGI("uid:%d, pid:%d", mClientUid, mClientPid);
}
    ...
    switch (msg->what()) {
        ...
        case kWhatHDCPNotify:
        {
            ...
            ALOGI("Saw HDCP notification code %d, ext1 %d, ext2 %d",
                    msgCode, ext1, ext2);
            ///M: prop for display check hdcp status @{
            property_set("media.wfd.hdcp","0");
            ///M: @}
            switch (msgCode) {
                case HDCPModule::HDCP_INITIALIZATION_COMPLETE:
                {
                    mHDCPInitializationComplete = true;
                    ///M: set prop when HDCP enabled @{
                    property_set("media.wfd.hdcp","1");
                    ///M @}
                ...
                ///M: if HDCP authentication failed, still trigger setup@{
...
status_t WifiDisplaySource::onSetupRequest(
    ...
    sp<PlaybackSession> playbackSession;
    ///M:When HDCP authenticaion fail,disable HDCP encrypt@{
    if (mUsingHDCP)
    {
       ALOGI("new playbackSession w/ HDCP");
       playbackSession =
            new PlaybackSession(
                    mOpPackageName, mNetSession, notify, mInterfaceAddr, mHDCP, mClientUid, mClientPid, mMediaPath.c_str());
    }
    else
    {
       ...
       playbackSession =
            new PlaybackSession(
                    mOpPackageName, mNetSession, notify, mInterfaceAddr, NULL, mClientUid, mClientPid, mMediaPath.c_str());
    }
    ///M @}
